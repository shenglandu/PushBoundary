...
def preprocessing():
    ...
    for file_name in listdir(data_path):
        if file_name.split('.')[1] == 'npy':
            ...
            # generate a container to store boundary related attributes
            boundary_data = np.stack((boundaries, dir_x, dir_y, dir_z), axis=1)

            print('Write to the output files...\n')

            # output results as new ply files
            process_file = join(process_path, file_name)
            new_data = np.concatenate((data, normals, boundary_data), axis=1)
            np.save(process_file, new_data.astype(np.float32))
