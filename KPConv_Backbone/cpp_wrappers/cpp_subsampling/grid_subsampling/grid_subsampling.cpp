...
void grid_subsampling(vector<PointXYZ>& original_points,
	...
	for (auto& v : data)
	{
	    PointXYZ pt = v.second.point * (1.0 / v.second.count);
		subsampled_points.push_back(pt);
		if (use_feature)
		{
            ...
            // Added by Du: get boundaries and normals
            else{
                // insert the color information r, g, b
                subsampled_features.insert(subsampled_features.end(),v.second.features.begin(),v.second.features.begin() + 4);
                // Get the point index with min distance;
                float minDistance = 1000000.0;
                float minDis_2_Boundary = 1000000.0;
                int n_minIdx = -1;
                int b_minIdx = -1;
                float current_boundary = 0.0;
                for (int i = 0; i < v.second.pts.size(); i++)
		        {
		            float distance = (pt - v.second.pts[i]).sq_norm();
		            if (distance < minDistance)
		            {
		                minDistance = distance;
		                n_minIdx = i;
		            }
		            if (v.second.boundaries[i] > 0.0)
		            {
		                if (current_boundary == 0.0)
		                    current_boundary = 1.0;
		            }
		        }
		        // insert the boundary and normals
		        subsampled_features.insert(subsampled_features.end(), v.second.normals.begin() + n_minIdx * 3, v.second.normals.begin() + (n_minIdx + 1) * 3);
		        subsampled_features.push_back(current_boundary);
		        subsampled_features.insert(subsampled_features.end(), v.second.dirs.begin() + n_minIdx * 3, v.second.dirs.begin() + (n_minIdx + 1) * 3);
            }
		}
...
//// Added by Du: boundary definition
//float Get_boundaryvalue(float distance, float original_boundaryValue)
//{
//    float sub_boundaryValue = 0.0;
//    if (distance <= 0.05)
//        sub_boundaryValue = original_boundaryValue;
//    else if (distance > 0.05 && distance <= 0.2)
//        sub_boundaryValue = original_boundaryValue - 0.2;
//    else if (distance > 0.2 && distance <= 0.3)
//        sub_boundaryValue = original_boundaryValue - 0.4;
//    else if (distance > 0.3 && distance <= 0.4)
//        sub_boundaryValue = original_boundaryValue - 0.6;
//    else if (distance > 0.4 && distance <= 0.5)
//        sub_boundaryValue = original_boundaryValue - 0.8;
//    else if (distance > 0.5)
//        sub_boundaryValue = original_boundaryValue - 1.0;
//
//    if (sub_boundaryValue < 0.0)
//        sub_boundaryValue = 0.0;
//
//    return sub_boundaryValue;
//}