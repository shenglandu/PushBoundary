...
if __name__ == '__main__':
    ...
    chosen_log = 'results/Log_2023-04-18_08-40-37'
    chkp_idx = None
    ...
    if config.dataset == 'S3DIS':
        ...
        test_sampler = S3DISSampler(test_dataset)
        collate_fn = S3DISCollate
    elif config.dataset == 'Sensat':
        test_dataset = SensatDataset(config, set='test', use_potentials=True)
        test_sampler = SensatSampler(test_dataset)
    ...
    if config.dataset == 'S3DIS':
        ...
        tester.sensat_segmentation_test(net, test_loader, config)