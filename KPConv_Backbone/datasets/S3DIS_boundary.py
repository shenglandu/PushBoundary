    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            if np.random.rand() > self.config.augment_color:
                input_colors *= 0
            input_features = np.hstack((input_colors, input_points[:, 2:] + center_point[:, 2:], input_normals)).astype(np.float32)
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                features = np.vstack((data['red'], data['green'], data['blue'], data['dis_boundary'], data['nx'],
                                          data['ny'], data['nz'], data['is_boundary'], data['dx'], data['dy'], data['dz'])).T # changed by Du
                labels = data['class']
                sub_points, sub_features, sub_labels = grid_subsampling(points,
                                                                      features=features,
                                                                      labels=labels,
                                                                      sampleDl=dl)
                sub_colors = sub_features[:, :3] / 255
                sub_labels = np.squeeze(sub_labels)
                sub_distances = np.squeeze(sub_features[:, 3].astype(np.float32))
                sub_normals = sub_features[:, 4:7]
                sub_boundaries = np.squeeze(sub_features[:, 7])
                sub_boundaries = (np.ceil(sub_boundaries)).astype(np.int32)
                sub_dirs = sub_features[:, 8:11]
                search_tree = KDTree(sub_points, leaf_size=10)
                with open(KDTree_file, 'wb') as f:
                    pickle.dump(search_tree, f)
                # Save ply
                write_ply(sub_ply_file,
                          [sub_points, sub_colors, sub_distances, sub_normals, sub_boundaries, sub_dirs, sub_labels],
                          ['x', 'y', 'z', 'red', 'green', 'blue', 'dis_boundary', 'nx', 'ny', 'nz', 'is_boundary', 'dx', 'dy', 'dz', 'class'])
