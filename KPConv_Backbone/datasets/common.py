...
class PointCloudDataset(Dataset):
    ...
    def segmentation_boundary_inputs(self,
                                     stacked_points,
                                     stacked_features,
                                     boundaries,
                                     dirs,
        ...
        # list of network inputs (Added by DU for the normals)
        li = input_points + input_neighbors + input_pools + input_upsamples + input_stack_lengths

        if in_feature_dim == 5 or in_feature_dim == 8:
            pass
        else:
            raise ValueError('Only accepted input dimensions is 5 or 8')

        li += [stacked_features, boundaries, dirs, labels]
        return li