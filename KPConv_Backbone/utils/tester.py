...
class ModelTester:
    ...
    def sensat_segmentation_test(self, net, test_loader, config, num_votes=100):
        """
        Test method for sensat segmentation models
        """
        # Testing parameters
        test_smooth = 0.95
        test_radius_ratio = 0.7
        ...
        # Initializing
        nc_model = config.num_classes
        self.test_probs = [np.zeros((n.shape[0], nc_model)) for n in test_loader.dataset.input_colors]
        self.test_bprobs = [np.zeros((l.shape[0], 2))
                                      for l in test_loader.dataset.input_colors]
        self.test_dpreds = [np.zeros((l.shape[0], 3))
                                      for l in test_loader.dataset.input_colors]
        ...
        while True:
            ...
            if last_min + 1 < new_min:
                ...
                if int(np.ceil(new_min)) % 1 == 0:
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        points = test_loader.dataset.load_evaluation_points(file_path)
                        probs = self.test_probs[i][test_loader.dataset.test_proj[i][0], :]
                        bprobs = self.test_bprobs[i][test_loader.dataset.test_proj[i][0], :]
                        dpreds = self.test_dpreds[i][test_loader.dataset.test_proj[i][0], :]
                        preds = test_loader.dataset.label_values[np.argmax(probs, axis=1)].astype(np.int32)
                        bpreds = test_loader.dataset.boundary_values[np.argmax(bprobs, axis=1)].astype(np.int32)
                        boundary_probs = bprobs[:, 1].astype(np.float32)
                        dpreds = dpreds.astype(np.float32)
                        cloud_name = file_path.split('/')[-1]
                        test_name = join(test_path, 'predictions', cloud_name)
                        write_ply(test_name,
                                  [points, preds, bpreds, boundary_probs, dpreds],
                                  ['x', 'y', 'z', 'preds', 'is_boundary_preds', 'is_boundary_probs', 'dx', 'dy', 'dz'])
                        # Save result files for submission
                        save_name = join(test_path, 'predictions', cloud_name.split('.')[0] + '.label')
